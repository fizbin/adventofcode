...
def process_snumber(snum):
    ...
    if m1:
        ...
        if lft:
            lfttxt = (
                lft.group(1) + str(int(lft.group(2)) + int(m1.group(3))) + lft.group(3)
            )
        if rgt:
            rgttxt = (
                rgt.group(1) + str(int(rgt.group(2)) + int(m1.group(4))) + rgt.group(3)
            )
        # print(f"Exploded: {lfttxt + '0' + rgttxt}")
    ...
    if m2:
        ...
        lft = rnum // 2
        rgt = (rnum + 1) // 2
        ans = f"{m2.group(1)}[{lft},{rgt}]{m2.group(3)}"
        # print(f"split: {ans}")
        return ans
    return snum
