...
workGoal :: (State -> [State]) -> (State -> Int) -> (State -> Bool) -> State -> State
workGoal nbF estF goalF = go . S.singleton
 where
  go :: S.Set State -> State
  go iStates =
   let dones = S.toList $ S.filter goalF iStates
   in
      case dones of
        (x:_) -> x
        _ -> go $ S.fromList $ concatMap nbF $ S.toList iStates 
...
main = do
  ...
  let part1 = workGoal (neigh pitchHeight pitchWidth grid) (estimate goal) ((== goal) . fst) (start, 0)
  print $ snd part1

  let part2a = workGoal (neigh pitchHeight pitchWidth grid) (estimate start) ((== start) . fst) part1
  let part2b = workGoal (neigh pitchHeight pitchWidth grid) (estimate goal) ((== goal) . fst) part2a
  print $ snd part2b
