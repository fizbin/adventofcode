...
def process_input():
    ...
    for (month, day, hour, minute, note) in get_merged_input():
        if 'begins shift' in note:
            mc = re.match(r'Guard #(\d+) begins shift', note)
            guard_num = int(mc.group(1))
            sleeping = False
    ...
    (max_sleep, max_sleep_guard, max_sleep_minute) = max(
        (s, g, m) for g in guard_total_minutes
        for (m, s) in enumerate(guard_minutes[g]))
