...
m = re.match(r"\s*Begin in state (\w+).\s+Perform a diagnostic checksum after (\d+) steps.\s+", data)
...
state_part_re = re.compile(r"If the current value is (\d+):\s+- Write the value (\d)\.\s+- Move one slot to the (\w+).\s+- Continue with state (\w+)\.\s+")
...
#     - Continue with state C.
    ...
    remaining = remaining[m.end(0):]
    machine[in_state] = [(write_if_0, move_if_0, state_if_0), (write_if_1, move_if_1, state_if_1)]
