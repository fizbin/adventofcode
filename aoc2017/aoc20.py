...
for line in data:
    ...
    particles.append(
        {
            "p": np.array(
                [int(m.group(1)), int(m.group(2)), int(m.group(3))], dtype=int
            ),
            "v": np.array(
                [int(m.group(4)), int(m.group(5)), int(m.group(6))], dtype=int
            ),
            "a": np.array(
                [int(m.group(7)), int(m.group(8)), int(m.group(9))], dtype=int
            ),
        }
    )
...
def at_time(particle, t):
    return tuple(
        particle["p"] + t * particle["v"] + ((t * (t + 1)) // 2) * particle["a"]
    )
