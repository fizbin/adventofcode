...
def play_game(recurse, deck1a, deck2a):
    ...
    while deck1 and deck2:
        ...
        if (roundn > 100000 == 0):
            roundn = 0
            print(f"alert {deck1} {deck2} @ {deck1a} {deck2a}")
        ...
        if recurse and top1 >= len(deck1) and top2 >= len(deck2):
            # print(f"Recursing {top1} {deck1} and {top2} {deck2}")
        ...
        if p1wins:
            ...
            deck2.append(top2)
            deck2.append(top1)
    # print(f"game over {deck1a} {deck2a} -> [{deck1}] [{deck2}]")
    return (deck1, deck2)
