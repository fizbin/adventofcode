...
dijkstraGenN' ::
  ...
  -> (state -> Bool)
  -> Maybe (state, Int)
dijkstraGenN' txn seeds comboFunc estimate accept =
  go
    S.empty
    (foldl' (\hp (a, b) -> IM.insertWith (++) b [(b, a)] hp) IM.empty seeds)
  where
    go !seen heap =
      case IM.minViewWithKey heap of
        Nothing -> Nothing
           ...
           in if val `S.member` seen
                then go seen heap'
                       ...
                       then Just (val, cost)
                                  ...
                                  (\(val', cost') ->
                                     let ncost = comboFunc cost cost'
                                      in ( comboFunc (estimate val') ncost
                                         , (ncost, val')))
                                    <$> txn val
                                ...
                                heap'' =
                                  foldl'
                                    (\hp (key, ent) -> IM.alter (ifn ent) key hp)
                                    heap'
                                    nxts
...
dijkstraGenN ::
  ...
  -> Maybe (state, cost)
dijkstraGenN txn seeds comboFunc estimate accept =
  go S.empty (foldl' (\hp (a, b) -> PM.insert (b, b, a) hp) PM.empty seeds)
  where
    go !seen heap =
      case PM.minView heap of
        Nothing -> Nothing
        Just ((_, cost, val), heap')
          ...
          | otherwise ->
            let nxts =
                  (\(val', cost') ->
                     let ncost = comboFunc cost cost'
                      in (comboFunc (estimate val') ncost, ncost, val'))
                    <$> txn val
