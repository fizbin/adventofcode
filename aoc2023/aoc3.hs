...
main = do
    ...
    let s = replicate (2 + length (head s0)) 'x' : ["x" ++ ln ++ "x" | ln <- s0] ++ [replicate (2 + length (head s0)) 'x']
    ...
    let goodnums = do
            ...
            (num, rest) <- if not (isDigit pstart) then reads start else []
    ...
    let asternums = do
            ...
            (asteri, asterj) <-
                [(rowidx, colidx - 1), (rowidx, colidx + numlen)]
                    ++ join
                        [ [(rowidx - 1, col), (rowidx + 1, col)] | col <- [colidx - 1 .. colidx + numlen]
                        ]
            guard $ M.lookup (asteri, asterj) chMap == Just '*'
            pure ((asteri, asterj), [num + 0 :: Int])
    let asterMap = M.fromListWith (++) asternums
    print $ sum $ map (\v -> head v * head (tail v)) $ filter ((== 2) . length) $ M.elems asterMap
