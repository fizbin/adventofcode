...
class IntCode():
    ...
    def yieldprog(self, inputf, debug_ip=False):
        self.memory._debug = debug_ip
        while True:
            if debug_ip:
                print('ip', self.ip)
...
if __name__ == '__main__':
    ...
    def do_input(inqueue):
        try:
            return inqueue.get(True, 1)  # the "1" makes us get both x and y
        except queue.Empty:
            return -1
    ...
    def progthread(n):
        inqueue = inqueues[n]
        inqueue.put(n)
        src = iter(IntCode(prog).yieldprog(lambda: do_input(inqueue),
                                           False))
        while True:
            try:
                dest = next(src)
                xval = next(src)
                yval = next(src)
                print("Emit", (xval, yval), "to", dest, "from", n)
                if dest == 255:
                    print("Done")
                    outq.put(yval)
                else:
                    inqueues[dest].put(xval)
                    inqueues[dest].put(yval)
            except StopIteration:
                break

    threads = []
    for thread_num in range(50):
        threads.append(threading.Thread(target=progthread, args=(thread_num,)))
