...
if __name__ == '__main__':
    ...
    def do_input():
        b = 0
        p = 0
        print("\x1b[1;1H                  \x1b[1;1H", end='')
        print(paint.get((-1, 0), 0))
        for my in range(25):
            for mx in range(45):
                ch = {0: ' ', 1: '#', 2: '.', 3: '-', 4: '@'}[
                    paint.get((mx, my), 0)]
    ...
    prog[0] = 2
    for output in IntCode(prog).evalprog(do_input, None):
        if output_type == 0:
            x = output
        elif output_type == 1:
            y = output
    ...
    do_input()
    print(paint[(-1, 0)])
    # print(len([foo for foo in paint if paint[foo] == 2]))
    # print((min(z for z in paint), max(z for z in paint)))
