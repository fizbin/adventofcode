...
def part1(grid, moves):
    ...
    if mydir == 1j: fdir = 0
    elif mydir == -1.0: fdir = 1
    elif mydir == -1j: fdir = 2
    elif mydir == 1.0: fdir = 3
...
def part2(grid, moves):
    ...
    for ((startspot, startdir), (nxtspot, nxtdir)) in jumps.items():
        if (nxtspot, -nxtdir) not in jumps:
            assert False, f"Couldn't jump from {(startspot, startdir)} to {(nxtspot, nxtdir)} and back"
        if jumps[(nxtspot, -nxtdir)] != (startspot, -startdir):
            assert False, f"jumps[{(startspot, startdir)}] = {(nxtspot, nxtdir)} but jumps[{(nxtspot, -nxtdir)}] = {jumps[(nxtspot, -nxtdir)] }"
        assert space.get(startspot + startdir, ' ') == ' ', f"Unnecessary jump at {(startspot, startdir)}"
