...
neighbors isPart1 grid (row, col) =
    ...
    in if isPart1 then [p | p <- allpos'', celev p <= succ nowelev]
...
main = do
    ...
    let addOneInt = let one = 1 :: Int in map (, one)
    let Just(final, dist) = dijkstra (addOneInt . neighbors True s) (startrow, startcol) (\p -> 'E' == getSpot p)
    print dist
    let Just(_, dist') = dijkstra (addOneInt . neighbors False s) final (\p -> getSpot p `elem` "aS")
    print dist'
