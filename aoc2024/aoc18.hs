...
blankMap :: Map (Int, Int) Char
blankMap = M.fromList [((x, y), '.') | x <- [0 .. 70], y <- [0 .. 70]]

fillMap :: [(Int, Int)] -> Map (Int, Int) Char
fillMap fallen = M.fromList ((,'#') <$> fallen) `M.union` blankMap
...
findThreshold lst = go 0 (length lst)
  where
    go a b | b - a <= 1 = lst !! a
    go a b =
      let m = (a + b) `div` 2
       ...
       in case dijkstra (((,one) <$>) <$> getNeighbors mymap) (0, 0) (== (70, 70)) of
            Nothing -> go a m
            _ -> go m b
...
main = do
  ...
  let one = 1 :: Int
  putStr "Part 1: "
  print $ snd $ fromJust $ dijkstra (((,one) <$>) <$> getNeighbors mymap) (0, 0) (== (70, 70))
  putStr "Part 2: "
